...
namespace {
  ...
  static const std::string sValue_Var_Directory( "var_directory" );
...
bool Load( const std::string& sFileName, Values& values ) {
  ...
  try {
    ...
    config.add_options()
      ...
      ( sValue_Var_Directory.c_str(), po::value<std::string>( &values.sVarDirectory )->default_value( "./" ), "var directory" )
    ...
    if ( !ifs ) {
      ...
      bOk &= parse<std::string>( sFileName, vm, sValue_Var_Directory, values.sVarDirectory );
